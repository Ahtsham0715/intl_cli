...
class FileScanner {
  ...
  Future<List<String>> scanAsync() async {
    ...
    if (!dir.existsSync()) {
      print(
          'ERROR: Directory not found in scanAsync: $normalizedDir (original: $directory)');
      throw FileSystemException(
          'Directory not found: $normalizedDir', normalizedDir);
    }
  ...
  List<String> scan() {
    ...
    if (!dir.existsSync()) {
      print(
          'ERROR: Directory not found in scan: $normalizedDir (original: $directory)');
      throw FileSystemException(
          'Directory not found: $normalizedDir', normalizedDir);
    }
  ...
  String _normalizeDirectory(String inputPath) {
    ...
    if (inputPath == 'internationalize' || inputPath == 'i18n') {
      print(
          'WARNING: Command name detected as directory in FileScanner, using "lib" instead');
      return 'lib';
    }
