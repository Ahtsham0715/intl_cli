...
class RefactorCommand extends Command {
  ...
  RefactorCommand() {
    argParser.addOption('dir',
        abbr: 'd',
        help: 'The directory to scan and refactor (defaults to "lib" if not specified)');
    ...
    argParser.addOption('package',
        abbr: 'p',
        help: 'Package name for import statements (autodetected if not specified)');
  ...
  void run() async {
    ...
    try {
      ...
      final effectivePackageName = packageName.isEmpty ? detectPackageName(dir) : packageName;
      ...
      if (e.message.contains('Directory not found') || e.message.contains('No such file')) {
        ...
        try {
          ...
          final newExtractedStrings = intl_cli.scanDirectory(dirPath);
          
          // Perform the refactoring
          // Detect package name automatically if not specified
          final effectivePackageNameForRecovery = packageName.isEmpty ? detectPackageName(dirPath) : packageName;
